...
def export_result(request):
  ...
  if data_format == 'hdfs-file': # Blocking operation, like downloading
    ...
    if not request.fs.exists(destination):
      raise PopupException(_('The destination does not exit!'))
    if request.fs.listdir_stats(destination):
      raise PopupException(_('The destination is not a empty directory!'))
    if is_embedded:
